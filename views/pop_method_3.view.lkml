...
view: order_items_method_3 {
  ...
  dimension: days_in_period {
    ...
    type: number
    sql: DATE_DIFF(DATE({% date_end current_date_range %}, 'US/Eastern'), DATE({% date_start current_date_range %}, 'US/Eastern'), DAY) ;;
  }
    ...
    sql:
      {% if current_date_range._is_filtered %}
        ...
        WHEN {% condition current_date_range %} ${created_raw} {% endcondition %}
          THEN DATE_DIFF(${created_date}, DATE({% date_start current_date_range %}, 'US/Eastern'), DAY) + 1
  ...
  dimension_group: date_in_period {
    ...
    type: time
    sql: TIMESTAMP(DATETIME(DATE_ADD(DATE({% date_start current_date_range %}, 'US/Eastern'), INTERVAL CAST(${day_in_period} - 1 AS INT64) DAY)), 'US/Eastern') ;;
    view_label: "_PoP"
    timeframes: [
      date,
      hour_of_day,
